...
export default function Index() {
  ...
  // Low stock products (stock > 0 and <= threshold), sorted alphabetically
  const lowStockProducts = products
    .filter((p: { stock: number; }) => p.stock > 0 && p.stock <= inventoryThreshold)
    .sort((a: { name: string; }, b: { name: any; }) => a.name.localeCompare(b.name));

  // Zero stock products, sorted alphabetically
  ...
  const handleProductClick = (productId: string) => {
    ...
    // Navigate to Shopify admin product page
              ...
              <BlockStack gap="200">
                ...
                <ul>
                  {lowStockProducts.map((product: Product) => (
                    <li
                      key={product.id}
                      onClick={() => typeof product.id === "string" && handleProductClick(product.id)}
                      style={{
                        cursor: "pointer",
